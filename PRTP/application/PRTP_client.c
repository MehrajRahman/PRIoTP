...
void on_subscription_ack_sids(struct iotmsg_node *node)
{
  for (; node != NULL; node = node->next)
  {
    ...
    case SUBSCRIBE_OK:
      ...
      if (parse_sensor_type(node->id) == CAMERA)
        add_sensor_dump(node->id);
      break;
...
void process_user_input(int sd, int *client_operation)
{
  ...
  case CLIENT_OPERATION_SUBSCRIBE_TO_SENSOR_NONRELIABLE:
    ...
    if (!sub_msg)
      sub_msg = create_iotmsg(SUBSCRIBE);
...
int main(int argc, char *argv[])
{
  ...
  while ((c = getopt(argc, argv, "hl:s:p:qaAr:gNH:")) != -1)
  {
    ...
    case 'r':
      if (!submsg)
        submsg = create_iotmsg(SUBSCRIBE);
  ...
  { /* List of sensor IDs given as cmd line parameters */
    uint32_t i;
