...
int __parse_bson_document(char* buf, uint32_t* bytes_left, struct srtp_packet** result, char* parent);
int parse_bson_document(char* buf, size_t len, struct srtp_packet** result)
{
  uint32_t bytes_left = len;
  struct srtp_packet* tmp = NULL;
  int ret = -1;


  if( !result ) result = &tmp;
  if( *result != NULL ) {
    log_debug(l, "BSON parser warning: possibly overwriting srtp_packet\n");
    *result = NULL;
  }
...
int __parse_bson_document(char* buf, uint32_t* bytes_left, struct srtp_packet** result, char* parent)
{
    ...
    double value_double;
    bool value_bool;
    /* Was used to hold the added sid into sids of srtp_packet
     * Now the approach is to assume that sids head in srtp_packet is pointing to the last added sid */
