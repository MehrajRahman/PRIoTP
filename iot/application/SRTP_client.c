...
void on_update_msg(int sd, struct srtp_packet* upd_msg)
{
...
{
  struct srtp_packet* sub_msg = NULL;
  ...
  if( sub_msg ) {
    add_pending_subscription((struct srtp_packet *) sub_msg);
        ...
        if( len == -1 ) {
          ...
          if( msg->type == LIST_RESPONSE ) {
           ...
           log_print(l, "---------Update message arrived------------ %d\n", client_operation!=CLIENT_OPERATION_UNSUBSCRIBE );
            ...
            on_subscription_ack_sids( ((struct srtp_packet *)msg)->data.blob );
...
int main(int argc, char *argv[])
{
  ...
  int c;
  int sd;
  struct srtp_packet* submsg = NULL;
  ...
  if( submsg ) {
    add_pending_subscription((struct srtp_packet *) submsg);
