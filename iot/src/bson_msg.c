...
int bson_iotmsg_int32(struct srtp_packet** msg, char* parent, char* key, int32_t value)
{
  ...
  if (strncmp(key, "type", 4) == 0) {
    if(*msg) {
      log_error(l, "BSON parser: srtp_packet type field appears twice!\n");
      return -1;
    }
    else {
...
int serialize_list(const struct srtp_packet* msg, char* buf)
{
...
int serialize_unsubscribe(const struct srtp_packet* msg, char* buf)
{
  uint32_t doc_len = 0, ow_len;
  char* cur = buf;
...
int serialize_list_response(const struct srtp_packet* msg, char* buf)
{
...
int serialize_subscribe(const struct srtp_packet* msg, char* buf)
{
  /* Can be used for itoa and writing array according to BSON spec
   * Now writing array with empty keys
   * int i;
  */
  ...
  log_debug(l, "---Sending subscribe by serializing srtp packet to bson bsg.------\n");
...
int serialize_subscribe_ack(const struct srtp_packet* msg, char* buf)
{
  /* Can be used for itoa and writing array according to BSON spec
   * Now writing array with empty keys
   * int i;
...
int serialize_update(const struct srtp_packet* msg, char* buf)
{
...
int serialize_update_ack(const struct srtp_packet* msg, char* buf)
{
...
int serialize_update_nack(const struct srtp_packet* msg, char* buf)
{
  uint32_t doc_len = 0, ow_len;
  char* cur = buf;
  ow_len = write_bson_doclen( cur, 0 ); MOVE_CUR
...
int serialize_keep_alive(const struct srtp_packet* msg, char* buf)
{
  uint32_t doc_len = 0, ow_len;
  char* cur = buf;
...
/* From struct srtp_packet to BSON buf
